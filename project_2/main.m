...
N3 = w4(1:N, :);
N4 = w5(1:N, :);
Xt = S + N1 + N2 + N3 + N4;  % complete signal
Nt = N1 + N2 + N3 + N4;      % noise sample
...
for k = 1:size(Nkl, 1)
    for l = 1:size(Nkl, 2)
        down = max(1, l - floor(kernel_len / 2));
        up = min(size(Nkl, 2), l + floor(kernel_len / 2));
        for i = down : up
            tmp_vec = Nkl(k, i, :);
...
for k = 1:size(Xkl, 1)
    for l = 1:size(Xkl, 2)
        down = max(1, l - floor(kernel_len / 2));
        up = min(size(Xkl, 2), l + floor(kernel_len / 2));
        for i = down : up
            tmp_vec = Xkl(k, i, :);
            tmp_vec = Rn_invsqrt(:, :, k, i) * tmp_vec(:); % pre-whiten
...
for k = 1:size(Xkl, 1)
    for l = 1:size(Xkl, 2)
        ...
        evc_1 = evc(:, index == 1);
        akl(:, k, l) = Rn_invsqrt(:, :, k, l) \ evc_1; % De-whiten
    end
end
...
fprintf("======Done calculating beamformers======\n");
